...
void trackPath(int state, word wrd, int wrdIdx)
				   
{
	...
	wrdIdx++;
	wrd[wrdIdx] = '\0';
	LeftLng[state].elem[LeftLng[state].size] = strdup(wrd);
	LeftLng[state].size++;
	for(i = 0, letter = MADFA_Sigma[0]; letter != '\0'; letter = MADFA_Sigma[++i]) 
	{
		if (MADFA_delta[state][i] != UNDEFINED_STATE)
		{
			wrd[wrdIdx] = letter;
			trackPath(MADFA_delta[state][i], wrd, wrdIdx);
		}
	}
}
...
void createLRLang()
{
	...
	for (i = 0; i < MADFA_q; i++)
	{
		...
		LeftLng[i].elem = calloc(Lng.size, sizeof(word));
	}
	trackPath(0, wrd, -1);
	for (i = 0; i < MADFA_q; i++)
	{
		LeftLng[i].elem[LeftLng[i].size] = NULL;
		ptr = realloc(LeftLng[i].elem, (LeftLng[i].size + 1) * sizeof(word));
		assertion(ptr != NULL, "TJX");
		LeftLng[i].elem = ptr;
	}
